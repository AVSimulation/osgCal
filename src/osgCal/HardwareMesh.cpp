                                  ...
                                  const osg::StateSet* stateSet ) const
{
    ...
    const osg::GLExtensions* glextensions = osg::GLExtensions::Get( state.getContextID(), true );
...
//    if ( deformed )
    if ( mesh->data->rigid == false && program )
    {
        ...
        if ( deformed )
        {
            ...
            glextensions->glUniformMatrix3fv( rotationMatricesAttrib,
                                               boneCount, GL_FALSE,
                                               &rotationMatrices[0][0] );
            glextensions->glUniform3fv( translationVectorsAttrib,
                                         boneCount,
    ...
    // -- Call display list --
	bool transparent = (stateSet->getRenderingHint() & osg::StateSet::TRANSPARENT_BIN) == osg::StateSet::TRANSPARENT_BIN;
    ...
    if ( transparent )
    {
        ...
        {   // ^ there can be no "frontFacing" in user shader
            glextensions->glUniform1f( frontFacing, 0.0 );
        }
        glCallList( dl );
        glCullFace( GL_BACK ); // then draw only front faces
        if ( frontFacing >= 0 )
        {
            glextensions->glUniform1f( frontFacing, 1.0 );
        }
        glCallList( dl );
    }
