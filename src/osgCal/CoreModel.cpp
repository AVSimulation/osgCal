...
CoreModel::load( const std::string& cfgFileNameOriginal,
				 ...
				 const std::string& meshCacheFile) throw (std::runtime_error)
{
    if ( calCoreModel )
    {
	...
	std::string meshcachefullpath = meshCacheFile;
	if (meshcachefullpath.empty())
		meshcachefullpath = meshesCacheFileName(cfgFileName);

	if (isFileExists(meshcachefullpath) == false)
    {
...
CoreModel::loadNoThrow( const std::string& cfgFileName,
						...
						const std::string& meshCacheFile) throw ()
{
    try
    {
		load(cfgFileName, ps, meshCacheFile);
        return true;
    }
